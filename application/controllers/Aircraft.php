...
class Aircraft extends MY_Controller {
	...
	function search(){
		...
		//echo $queryString;
		//die();		
		$url = 'http://api.ofacanalyzer.com/serviceSearch.asmx/OFAC_Search?ofac_idF=803438&name_word_1F='.$post['search_terms'].'&name_word_2F=&name_word_3F=&name_word_4F=&name_word_5F=&search_dobF=21-02-2000&add_numF=&add_streetF=&add_cityF='.$post['city'].'&add_st_provF='.$post['state'].'&add_zipF='.$post['zip'].'&add_countryF='.$post['country'].'&search_commentsF='.$post['alias'].'';
